...
```bash
#!/bin/bash
# Script file: /root/monitor_incusd.sh
# Function: Check the CPU usage of the incusd process every minute, and restart the service if it exceeds 80% for 3 consecutive minutes
...
# Get CPU usage of the incusd process (taking the maximum value of all matching items)
...
if [ "$(echo "$CPU_USAGE > 80.0" | bc -l)" -eq 1 ]; then
    ...
    echo "$CURRENT_TIME - incusd process highest CPU usage: $CPU_USAGE%, counter: $COUNTER" >> "$LOG_FILE"
    ...
    echo "0" > "$COUNTER_FILE"
    echo "$CURRENT_TIME - incusd process highest CPU usage: $CPU_USAGE%, status normal" >> "$LOG_FILE"
fi
```
